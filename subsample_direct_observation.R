...
#save(sub_df, file = "sub_df_2week_datetimevariation.RData")
#save(subgroup_data, ts, lion_ids, xs, ys, file = "subgroup_data.RData")

#want to know the effect of start date and time of recording on the subgrouping patterns
...
#quantify the variance of subgroup size and number of subgroups in start date
boxplot(sub_df$sub_count ~ as.numeric(sub_df$month))

ggplot(sub_df, aes(month, sub_count, fill = as.factor(focal_ind)))+
  geom_boxplot()
...
#here we will use the full subgroup data matrix to subsampling through each individual through different day intervals, at different times, as well as increasing the number of individuals sampled to determine how many individuals and at what sampling regime is needed to extract subgrouping dynamics similar to the true 15 minute data



#different day sampling regimes
#different time sampling regimes



n <- c(1:7, 14, 30)

seq(from = 1, to= n_times, by = n)
